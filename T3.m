...
% T3 sweeps through the elements and calls the functions that generate the
% T3 blocks of the free vector (RHS) of the solving system. T3 blocks are
% only calculated for elements that contain Neumann boundaries. For all
% other elements, the T3 blocks are filled with zeros.
%
% T3 is called by MAIN***. It receives as input data the Edges, Loops and
...
% and weights. It returns to MAIN*** the RHS vector with the T3 blocks of
...
%
%
% T3 computes the internal product between the displacement basis U3 of the 
% element, expressed in a normal-tangential referential, and the applied 
% tractions on its Neumann boundaries. 
% * basis U3 is written in the polar (r,th) referential as,
%        U3 = | Ur | = r^n * | [k3*(n-3) + k2*(n-1)] * sin[(n-1)*th] |
%             | Ut |         | [k3*(n+3) + k2*(n+1)] * cos[(n-1)*th] |
% where n is the line of the current entry in vector T3, and k2 and k3 are 
% stiffness coefficients (see % INPUTPROC***). Basis U3 is subsequently 
...
for ii=1:length(Loops.area)
    ...
    % Computing the T3 vector of element ii. Function T3_VECTOR_I is a
    % local function (see below).
    TX3i = T3_Vector_i(LocLoop, Edges, BConds, abscissa, weight);
    
    % Inserting the T3 block in the RHS vector. The insertion is made at
    % line Loops.insert(ii,3).
    RHS(LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = TX3i;
    
end

end

function T3i = T3_Vector_i(LocLoop, Edges, BConds,abscissa, weight)
% T3_VECTOR_I local function computes the T3 vector of the LocLoop element. 
% The Neumann sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the T3 block
T3i = zeros(LocLoop.dim(1),1);

% n-1 denotes the current line (the term of the displacement basis is a
% rigid body term for n = 1)
n = 2:LocLoop.order(1);
...
for jj = 1:length(LocLoop.edges)  
    ...
    if strcmpi(Edges.type(id),'N')    
        ...
        % Performing the side integration and updating the T3 vector
        T3i = T3i + L/2 * sum(bsxfun(@times,T3i2D,weight.'),2); 
    end
end
end