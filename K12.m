function LHS = K12(Edges, Loops, LHS, abscissa, weight)
% sweeps through the elements and calls the functions that generate the
% K12 stiffness matrix in the LHS.
...
for jj = 1:length(LocLoop.edges)  % contour integration
   ...
   % base Uhs e Shs (12);
    Ur = R.^N.*sin((N+1).*T) ;
    Ut = R.^N.*cos((N+1).*T) ;
    Sr = 2*LocLoop.materials(5)*M.*R.^(M-1).*cos((M+1).*T);
    St = 2*LocLoop.materials(5)*M.*R.^(M-1).*-cos((M+1).*T);
    Srt= 2*LocLoop.materials(5)*M.*R.^(M-1).*-sin((M+1).*T);
    ...
    K12i = K12i + L/2 * sum(bsxfun(@times,NUS,w3D),3); % computes the integral
