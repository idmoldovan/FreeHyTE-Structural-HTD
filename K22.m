...
% K22 sweeps through the elements and calls the functions that generate the
% K22 block of the stiffness matrix in the LHS.
%
% K22 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the K22 blocks of
...
%
%
% K22 computes the internal product between the following bases, expressed
...
% term in matrix K22, k3 is the shear stiffness coefficient (see 
...
for ii=1:length(Loops.area)
    ...
    % Computing the K22 matrix of element ii. Function K22_MATRIX_I is a
    ...
    % at line Loops.insert(ii,2) and column Loops.insert(ii,2).
...
% K22_MATRIX_I local function computes the K22 stiffness block of the 
% LocLoop element. The sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the K22 block
K22i = zeros(LocLoop.dim(2));
...
for jj = 1:length(LocLoop.edges)  
    ...
    % Performing the side integration and updating the K22 matrix
    w3D(1,1,:) = weight;
    K22i = K22i + L/2 * sum(bsxfun(@times,NUS,w3D),3); 
