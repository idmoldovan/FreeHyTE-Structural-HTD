function LHS = K44(Edges, Loops, LHS, abscissa, weight)
% sweeps through the elements and calls the functions that generate the
% K44 stiffness matrix in the LHS.
...
for ii=1:length(Loops.area)
    ...
    K44i = K44_Matrix_i(LocLoop, Edges, abscissa, weight);
    
    % Inserting the matrix in the global LHS matrix
    LHS(LocLoop.insert(4):LocLoop.insert(4)+LocLoop.dim(4)-1,...
        LocLoop.insert(4):LocLoop.insert(4)+LocLoop.dim(4)-1) = K44i;
    
end

end

function K44i = K44_Matrix_i(LocLoop, Edges, abscissa, weight)

% computes the Ddd matrix of element ii

K44i = zeros(LocLoop.dim(1));
...
for jj = 1:length(LocLoop.edges)  % contour integration
   ...
   % base Uhs e Shs (44);
    ...
    K44i = K44i + L/2 * sum(bsxfun(@times,NUS,w3D),3); % computes the integral
