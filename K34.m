...
% K34 sweeps through the elements and calls the functions that generate the
% K34 block of the stiffness matrix in the LHS.
%
% K34 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the K34 blocks of
...
%
%
% K34 computes the internal product between the following bases, expressed
...
% term in matrix K34, k2 and k3 are stiffness coefficients (see 
...
for ii=1:length(Loops.area)
    ...
    % Computing the K34 matrix of element ii. Function K34_MATRIX_I is a
    ...
    % at line Loops.insert(ii,3) and column Loops.insert(ii,4).
...
% K34_MATRIX_I local function computes the K34 stiffness block of the 
% LocLoop element. The sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the K34 block
K34i = zeros([LocLoop.dim(3),LocLoop.dim(4)]);

% n-1 denotes the current line; m is the current column. The term of the 
% stress basis is null for n = 1 and thus excluded from the basis.
n = 2:LocLoop.order(1);
m = 1:LocLoop.order(1);
...
for jj = 1:length(LocLoop.edges)  
    ...
    % Performing the side integration and updating the K34 matrix
    w3D(1,1,:) = weight;
