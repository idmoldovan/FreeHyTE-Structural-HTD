function LHS = K22(Edges, Loops, LHS, abscissa, weight)
% sweeps through the elements and calls the functions that generate the
% K22 stiffness matrix in the LHS.
...
for ii=1:length(Loops.area)
    ...
    K22i = K22_Matrix_i(LocLoop, Edges, abscissa, weight);
    
    % Inserting the matrix in the global LHS matrix
    LHS(LocLoop.insert(2):LocLoop.insert(2)+LocLoop.dim(2)-1,...
        LocLoop.insert(2):LocLoop.insert(2)+LocLoop.dim(2)-1) = K22i;
    
end

end

function K22i = K22_Matrix_i(LocLoop, Edges, abscissa, weight)

% computes the Ddd matrix of element ii

K22i = zeros(LocLoop.dim(1));
...
for jj = 1:length(LocLoop.edges)  % contour integration
   ...
   % base Uhs e Shs (22);
    ...
    K22i = K22i + L/2 * sum(bsxfun(@times,NUS,w3D),3); % computes the integral
