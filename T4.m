function RHS = T4(Edges, Loops, BConds,RHS, abscissa, weight)
% T4 sweeps through the elements and calls the functions that generate the
% T4 blocks of the free vector (RHS) of the solving system. T4 blocks are
% only calculated for elements that contain Neumann boundaries. For all
% other elements, the T4 blocks are filled with zeros.
%
% T4 is called by MAIN***. It receives as input data the Edges, Loops and
...
% and weights. It returns to MAIN*** the RHS vector with the T4 blocks of
...
%
%
% T4 computes the internal product between the displacement basis U4 of the 
% element, expressed in a normal-tangential referential, and the applied 
% tractions on its Neumann boundaries. 
% * basis U4 is written in the polar (r,th) referential as,
%        U4 = | Ur | = r^n * | [k3*(n-3) + k2*(n-1)] * cos[(n-1)*th] |
%             | Ut |         |-[k3*(n+3) + k2*(n+1)] * sin[(n-1)*th] |
% where n is the line of the current entry in vector T4, and k2 and k3 are 
% stiffness coefficients (see % INPUTPROC***). Basis U4 is subsequently 
% transformed to the normal-tangential referential,
%        Un = nr*Ur + nt*Ut
%        Ut = -nt*Ur + nr*Ut
...
for ii=1:length(Loops.area)
    ...
    LocLoop = struct('id',ii,'edges',Loops.edges(ii,:), 'center',...
        ...
        'insert',Loops.insert(ii,4),'dim',Loops.dim(ii,4),...
        'materials',Loops.materials(ii,:));
    
    % Computing the T4 vector of element ii. Function T4_VECTOR_I is a
    % local function (see below).
    T4i = T4_Vector_i(LocLoop, Edges, BConds, abscissa, weight);
    
    % Inserting the T4 block in the RHS vector. The insertion is made at
    % line Loops.insert(ii,4).
    RHS(LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = T4i;
    
end

end

function T4i = T4_Vector_i(LocLoop, Edges, BConds,abscissa, weight)
% T4_VECTOR_I local function computes the T4 vector of the LocLoop element. 
% The Neumann sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the T4 block
T4i = zeros(LocLoop.dim(1),1);
...
for jj = 1:length(LocLoop.edges)  
    ...
    if strcmpi(Edges.type(id),'N')    
        ...
        % Performing the side integration and updating the T4 vector
        T4i = T4i + L/2 * sum(bsxfun(@times,T4i2D,weight.'),2); % computes the integral
    end
end
end