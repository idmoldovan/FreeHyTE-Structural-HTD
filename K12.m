...
% K12 sweeps through the elements and calls the functions that generate the
% K12 block of the stiffness matrix in the LHS.
%
% K12 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the K12 blocks of
...
%
%
% K12 computes the internal product between the following bases, expressed
...
% generated by even harmonic potentials,
%        S2 = | Sr | = 2 * k3 * m * r^(m-1) * | cos[(m+1)*th] |
%             | St |                          |-cos[(m+1)*th] |
%             | Srt|                          |-sin[(m+1)*th] |
...
% term in matrix K12, k3 is the shear stiffness coefficient (see 
...
for ii=1:length(Loops.area)
    ...
    % Computing the K12 matrix of element ii. Function K12_MATRIX_I is a
    ...
    % at line Loops.insert(ii,1) and column Loops.insert(ii,2).
...
% K12_MATRIX_I local function computes the K12 stiffness block of the 
% LocLoop element. The sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the K12 block
K12i = zeros([LocLoop.dim(1),LocLoop.dim(2)]);
...
for jj = 1:length(LocLoop.edges)  
    ...
    % Performing the side integration and updating the K12 matrix
    w3D(1,1,:) = weight;
