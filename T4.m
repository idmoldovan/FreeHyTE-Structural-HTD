...
for ii=1:length(Loops.area)
    ...
    X4i = X4_Vector_i(LocLoop, Edges, BConds, abscissa, weight);
    
    % Inserting the vector in the global RHS vector
    RHS(LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = X4i;
    
end

end

function X4i = X4_Vector_i(LocLoop, Edges, BConds,abscissa, weight)

% computes the Xd vector of element ii

X4i = zeros(LocLoop.dim(1),1);
...
%           tn=1; ttg=1;       
        X4i2D = Un.*tn + Utg.*ttg; % this creates the 2D Xdi2D matrix,
                                            % one Gauss point per column
        
        X4i = X4i + L/2 * sum(bsxfun(@times,X4i2D,weight.'),2); % computes the integral
