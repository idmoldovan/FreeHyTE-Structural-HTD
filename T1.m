...
for ii=1:length(Loops.area)
    ...
    X1i = X1_Vector_i(LocLoop, Edges, BConds, abscissa, weight);
    
    % Inserting the vector in the global RHS vector
    RHS(LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = X1i;
    
end

end

function X1i = X1_Vector_i(LocLoop, Edges, BConds,abscissa, weight)

% computes the Xd vector of element ii

X1i = zeros(LocLoop.dim(1),1);
...
%         tn = 1; ttg=1;
        X1i2D = Un.*tn + Utg.*ttg; % this creates the 2D Xdi2D matrix,
                                            % one Gauss point per column
        
        X1i = X1i + L/2 * sum(bsxfun(@times,X1i2D,weight.'),2); % computes the integral
