...
% K11 sweeps through the elements and calls the functions that generate the
% K11 block of the stiffness matrix in the LHS.
%
% K11 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the K11 blocks of
...
%
%
% K11 computes the internal product between the following bases, expressed
% in a polar (r,th) referential:
% * the displacement basis U1, generated by odd harmonic potentials,
%        U1 = | Ur | = r^n * | sin[(n+1)*th] |
%             | Ut |         | cos[(n+1)*th] |
% * the boundary traction basis N * S1, where S1 is the stress basis
% generated by odd harmonic potentials,
%        S1 = | Sr | = 2 * k3 * m * r^(m-1) * | sin[(m+1)*th] |
%             | St |                          |-sin[(m+1)*th] |
%             | Srt|                          | cos[(m+1)*th] |
...
% term in matrix K11, k3 is the shear stiffness coefficient (see 
...
for ii=1:length(Loops.area)
    ...
    % Computing the K11 matrix of element ii. Function K11_MATRIX_I is a
...
% K11_MATRIX_I local function computes the K11 stiffness block of the 
% LocLoop element. The sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the K11 block
...
for jj = 1:length(LocLoop.edges)  
    ...
    % Performing the side integration and updating the K11 matrix
    w3D(1,1,:) = weight;
