function RHS = T1(Edges, Loops, BConds, RHS, abscissa, weight)
% T1 sweeps through the elements and calls the functions that generate the
% T1 blocks of the free vector (RHS) of the solving system. T1 blocks are
% only calculated for elements that contain Neumann boundaries. For all
% other elements, the T1 blocks are filled with zeros.
%
% T1 is called by MAIN***. It receives as input data the Edges, Loops and
...
% and weights. It returns to MAIN*** the RHS vector with the T1 blocks of
...
%
%
% T1 computes the internal product between the displacement basis U1 of the 
% element, expressed in a normal-tangential referential, and the applied 
% tractions on its Neumann boundaries. 
% * basis U1 is written in the polar (r,th) referential as,
%        U1(r,th) = | Ur | = r^n * | sin[(n+1)*th] |
%                   | Ut |         | cos[(n+1)*th] |
% where n is the line of the current entry in vector T1, and transformed to 
...
for ii=1:length(Loops.area)
    ...
    LocLoop = struct('id',ii,'edges',Loops.edges(ii,:), 'center',...
        ...
        'insert',Loops.insert(ii,1),'dim',Loops.dim(ii,1),...
        'materials',Loops.materials(ii,:));
    
    % Computing the T1 vector of element ii. Function T1_VECTOR_I is a
    % local function (see below).
    T1i = T1_Vector_i(LocLoop, Edges, BConds, abscissa, weight);
    
    % Inserting the T1 block in the RHS vector. The insertion is made at
    % line Loops.insert(ii,1).
    RHS(LocLoop.insert:LocLoop.insert+LocLoop.dim-1) = T1i;
    
end

end

function T1i = T1_Vector_i(LocLoop, Edges, BConds,abscissa, weight)
% T1_VECTOR_I local function computes the T1 vector of the LocLoop element. 
% The Neumann sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the T1 block
T1i = zeros(LocLoop.dim(1),1);
...
for jj = 1:length(LocLoop.edges)  
    ...
    if strcmpi(Edges.type(id),'N')
        ...
        % Performing the side integration and updating the T1 vector
        T1i = T1i + L/2 * sum(bsxfun(@times,T1i2D,weight.'),2); 
    end
end
end