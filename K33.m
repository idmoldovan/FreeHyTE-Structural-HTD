function LHS = K33(Edges, Loops, LHS, abscissa, weight)
% sweeps through the elements and calls the functions that generate the
% K33 stiffness matrix in the LHS.
...
for ii=1:length(Loops.area)
    ...
    K33i = K33_Matrix_i(LocLoop, Edges, abscissa, weight);
    
    % Inserting the matrix in the global LHS matrix
    LHS(LocLoop.insert(3):LocLoop.insert(3)+LocLoop.dim(3)-1,...
        LocLoop.insert(3):LocLoop.insert(3)+LocLoop.dim(3)-1) = K33i;
    
end

end

function K33i = K33_Matrix_i(LocLoop, Edges, abscissa, weight)

% computes the Ddd matrix of element ii

K33i = zeros(LocLoop.dim(3));
...
for jj = 1:length(LocLoop.edges)  % contour integration
   ...
   % base Uhs e Shs (33);
    ...
    K33i = K33i + L/2 * sum(bsxfun(@times,NUS,w3D),3); % computes the integral
