...
% K33 sweeps through the elements and calls the functions that generate the
% K33 block of the stiffness matrix in the LHS.
%
% K33 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the K33 blocks of
...
%
%
% K33 computes the internal product between the following bases, expressed
...
% term in matrix K33, k2 and k3 are stiffness coefficients (see 
...
for ii=1:length(Loops.area)
    ...
    % Computing the K33 matrix of element ii. Function K33_MATRIX_I is a
    ...
    % at line Loops.insert(ii,3) and column Loops.insert(ii,3).
...
% K33_MATRIX_I local function computes the K33 stiffness block of the 
% LocLoop element. The sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the K33 block
K33i = zeros(LocLoop.dim(3));

% n-1 denotes the current line; m-1 is the current column. The term of the 
% stress basis is null for n(m) = 1 and thus excluded from the basis.
n = 2:LocLoop.order(1);
m = 2:LocLoop.order(1);
...
for jj = 1:length(LocLoop.edges)  
    ...
    % Performing the side integration and updating the K33 matrix
    w3D(1,1,:) = weight;
