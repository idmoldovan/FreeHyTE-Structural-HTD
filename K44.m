...
% K44 sweeps through the elements and calls the functions that generate the
% K44 block of the stiffness matrix in the LHS.
%
% K44 is called by MAIN***. It receives as input data the Edges and Loops
...
% and weights. It returns to MAIN*** the LHS matrix with the K44 blocks of
...
%
%
% K44 computes the internal product between the following bases, expressed
...
% term in matrix K44, k2 and k3 are stiffness coefficients (see 
...
for ii=1:length(Loops.area)
    ...
    % Computing the K44 matrix of element ii. Function K44_MATRIX_I is a
    ...
    % at line Loops.insert(ii,4) and column Loops.insert(ii,4).
...
% K44_MATRIX_I local function computes the K44 stiffness block of the 
% LocLoop element. The sides are mapped to a [-1,1] interval to perform the 
% integrations.

%% Initialization 
% Initialization of the K44 block
K44i = zeros(LocLoop.dim(4));
...
for jj = 1:length(LocLoop.edges)  
    ...
    % Performing the side integration and updating the K44 matrix
    w3D(1,1,:) = weight;    
