function LHS = K24(Edges, Loops, LHS, abscissa, weight)
% sweeps through the elements and calls the functions that generate the
% K24 stiffness matrix in the LHS.
...
function K24i = K24_Matrix_i(LocLoop, Edges, abscissa, weight)

% computes the Ddd matrix of element ii

K24i = zeros(LocLoop.dim(1));
...
for jj = 1:length(LocLoop.edges)  % contour integration
   ...
   % base Uhs e Shs (24);
    ...
    K24i = K24i + L/2 * sum(bsxfun(@times,NUS,w3D),3); % computes the integral
