function LHS = K23(Edges, Loops, LHS, abscissa, weight)
% sweeps through the elements and calls the functions that generate the
% K23 stiffness matrix in the LHS.
...
for ii=1:length(Loops.area)
    ...
    K23i = K23_Matrix_i(LocLoop, Edges, abscissa, weight);
    
    % Inserting the matrix in the global LHS matrix
    LHS(LocLoop.insert(2):LocLoop.insert(2)+LocLoop.dim(2)-1,...
        LocLoop.insert(3):LocLoop.insert(3)+LocLoop.dim(3)-1) = K23i;
    % Inserting the transposed of the matrix in the global LHS matrix
    LHS(LocLoop.insert(3):LocLoop.insert(3)+LocLoop.dim(3)-1,...
        LocLoop.insert(2):LocLoop.insert(2)+LocLoop.dim(2)-1) = K23i';
    
end

end

function K23i = K23_Matrix_i(LocLoop, Edges, abscissa, weight)

% computes the Ddd matrix of element ii

K23i = zeros([LocLoop.dim(2),LocLoop.dim(3)]);
...
for jj = 1:length(LocLoop.edges)  % contour integration
   ...
   % base Uhs e Shs (23);
    Ur = R.^N.*cos((N+1).*T) ;
    Ut = R.^N.*-sin((N+1).*T) ;
    Sr = 2*LocLoop.materials(5)*(LocLoop.materials(4)+LocLoop.materials(5))*M.*R.^(M-1).*((M-3).*sin((M-1).*T));
    St = 2*LocLoop.materials(5)*(LocLoop.materials(4)+LocLoop.materials(5))*M.*R.^(M-1).*(-(M+1).*sin((M-1).*T));
    Srt= 2*LocLoop.materials(5)*(LocLoop.materials(4)+LocLoop.materials(5))*M.*R.^(M-1).*((M-1).*cos((M-1).*T));
    ...
    K23i = K23i + L/2 * sum(bsxfun(@times,NUS,w3D),3); % computes the integral
