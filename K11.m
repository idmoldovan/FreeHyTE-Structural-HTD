function LHS = K11(Edges, Loops, LHS, abscissa, weight)
% sweeps through the elements and calls the functions that generate the
% K11 stiffness matrix in the LHS.
...
for ii=1:length(Loops.area)
    ...
    K11i = K11_Matrix_i(LocLoop, Edges, abscissa, weight);
    
    % Inserting the matrix in the global LHS matrix
    LHS(LocLoop.insert(1):LocLoop.insert(1)+LocLoop.dim(1)-1,...
        LocLoop.insert(1):LocLoop.insert(1)+LocLoop.dim(1)-1) = K11i;
    
end

end

function K11i = K11_Matrix_i(LocLoop, Edges, abscissa, weight)

% computes the Ddd matrix of element ii

K11i = zeros(LocLoop.dim(1));
...
for jj = 1:length(LocLoop.edges)  % contour integration
   ...
   % base Uhs e Shs (11);
    Ur = R.^N.*sin((N+1).*T) ;
    Ut = R.^N.*cos((N+1).*T) ;
    Sr = 2*LocLoop.materials(5)*M.*R.^(M-1).*sin((M+1).*T);
    St = 2*LocLoop.materials(5)*M.*R.^(M-1).*-sin((M+1).*T);
    Srt= 2*LocLoop.materials(5)*M.*R.^(M-1).*cos((M+1).*T);
    ...
    K11i = K11i + L/2 * sum(bsxfun(@times,NUS,w3D),3); % computes the integral
